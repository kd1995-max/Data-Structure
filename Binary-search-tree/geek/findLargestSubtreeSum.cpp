...
int findLargestSubtreeSumUtil(Node* root, int& ans)
{
    ...
    int parent = root->key;
    int lh = findLargestSubtreeSumUtil(root->left, ans);
    int rh = findLargestSubtreeSumUtil(root->right, ans);

    int currSum = parent + lh + rh;
...
int main()
{
	...
	*/
	...
	cout << endl << findLargestSubtreeSum(root) << endl;
	return 0;
}
